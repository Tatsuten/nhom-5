#define _CRT_SECURE_NO_WARNINGS // loại bỏ các cảnh báo an toàn của trình biên dịch
#include <iostream>
#include <string>
#include <conio.h>	//hỗ trợ các hàm giúp bạn thực hiện các thao tác input hoặc output từ màn hình console
#include <iomanip>	//Tiêu đề cung cấp trình điều khiển tham số
#include <stdlib.h> //chuẩn định nghĩa 4 kiểu biến, một số macro và các hàm đa dạng để thực hiện các tính năng chung.
...
BOOL WINAPI CtrlHandler(DWORD fdwCtrlType)
{
	switch (fdwCtrlType)
	{
		// CTRL-CLOSE: xác nhận người dùng muốn thoát.
